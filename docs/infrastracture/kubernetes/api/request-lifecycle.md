...
**「望ましい状態を宣言し、あとはKubernetesに任せる」**——これが宣言的な状態管理です。ユーザーやコントローラが逐一「Podを1個起動しろ」「今度は2個目を起動しろ」と命令する必要はなく、controllerが継続的に spec と status を見比べて、差分を埋め続けます。

> **用語について**: この「spec と status の差分を埋め続けるループ」のことを、Kubernetesのコントローラ実装（後の章で扱う `client-go` や `controller-runtime` など）では慣習的に **reconcileループ** と呼びます。ただし本章（Programming Kubernetes 第2章）の原文自体にはこの単語は登場せず、あくまで一般的なKubernetesのコントローラ用語として補足しています。
...
1. 「不安定なクラスタ」モードでは、まず一定確率でPodがランダムにクラッシュする（外部要因による障害のシミュレーション。これが起きた回はここで終わり、2以降には進まない）
2. `status.replicas`（現在のPod数）と `spec.replicas`（望ましいPod数）を比較する
3. `status < spec` なら、不足分のPodを1つ作成する
4. `status > spec` なら、余分なPodを1つ削除する
5. 一致していれば何もしない
